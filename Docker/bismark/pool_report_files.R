...
write(sprintf("Sequence pairs analysed in total:\t %d",sum(pd$total_reads)),printer,append=T)
write(sprintf("Number of paired-end alignments with a unique best hit:\t %d",sum(pd$uniquely_aligned_reads)),printer,append=T)
write(sprintf("Sequence pairs did not map uniquely:\t %d",sum(pd$non_uniquely_aligned_reads)),printer,append=T)
write(sprintf("Sequence pairs did not map uniquely:\t %d",sum(pd$non_uniquely_aligned_reads)),printer,append=T)
write(sprintf("CT/GA/CT:\t %d",sum(pd$OT_reads)),printer,append=T)
write(sprintf("CT/GA/GA:\t %d",sum(pd$OB_reads)),printer,append=T)
write(sprintf("GA/CT/CT:\t %d",sum(pd$CTOT_reads)),printer,append=T)
write(sprintf("GA/CT/GA:\t %d",sum(pd$CTOB_reads)),printer,append=T)
write(sprintf("Total methylated C's in CHG context:\t %d",sum(pd$CHG_meth)),printer,append=T)
write(sprintf("Total methylated C's in CHH context:\t %d",sum(pd$CHH_meth)),printer,append=T)
write(sprintf("Total unmethylated C's in CHG context:\t %d",sum(pd$CHG_unmeth)),printer,append=T)
write(sprintf("Total unmethylated C's in CHH context:\t %d",sum(pd$CHH_unmeth)),printer,append=T)
close(printer) 